 ...
 * <ul>
 * 
 * <li>The first long integer of the file represents the offset of the last written checkpoint, or -1 if there are no
 * checkpoints
 * 
 ...
 * 
 * <li>Each log record ends with a long integer file offset representing the position in the log file where the record
 * began.
 * 
 ...
 * 
 * <li>UPDATE RECORDS consist of two entries, a before image and an after image. These images are serialized Page
 * objects, and can be accessed with the LogFile.readPageData() and LogFile.writePageData() methods. See LogFile.print()
 * for an example.
 * 
 * <li>CHECKPOINT records consist of active transactions at the time the checkpoint was taken and their first log record
 * on disk. The format of the record is an integer count of the number of transactions, as well as a long integer
 * transaction id and a long integer first record offset for each active transaction.
 * 
 * </ul>
 */
...
public class LogFile {
    ...
    public synchronized void logWrite(TransactionId tid, Page before, Page after) throws IOException {
         ...
         * 
         * record type transaction id before page data (see writePageData) after page data start offset
         */
    ...
    public void rollback(TransactionId tid) throws NoSuchElementException, IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                undoTransaction(tid.getId());
            }
        }
    }

    private void replacePage(Page page) throws NoSuchElementException, IOException {
    ...
    private void undoTransactions(Set<Long> transactions) throws IOException {
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                long minRecord = Long.MAX_VALUE;

                for (long tid : transactions) {
                    Long record = tidToFirstLogRecord.get(tid);
                    if (record == null) {
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                // Move to checkpoint if there is one, otherwise already in right place.
                if (checkpoint != NO_CHECKPOINT_ID) {
                    raf.seek(checkpoint);
                    if (raf.readInt() != CHECKPOINT_RECORD) {
                        throw new IllegalStateException("First long didn't point to checkpoint");
                    }

                    raf.readLong();

                    // Add all running transactions.
                    int num = raf.readInt();
                    for (int i = 0; i < num; i++) {
                        long tid = raf.readLong();
