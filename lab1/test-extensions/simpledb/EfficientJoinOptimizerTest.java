...
public class EfficientJoinOptimizerTest extends SimpleDbTestBase {
     ...
     * @param columns
     *            Each entry in tuples[] must have "columns == tuples.get(i).size()"
     * @param colPrefix
     *            String to prefix to the column names (the columns are named after their column number by default)
     * @return a new HeapFile containing the specified tuples
     * @throws IOException
     *             if a temporary file can't be created to hand to HeapFile to open and read its data
     */
    ...
    public void setUp() throws Exception {
        ...
        tuples1 = new ArrayList<ArrayList<Integer>>();
        f1 = SystemTestUtil.createRandomHeapFile(10, 1000, 20, null, tuples1, "c");
        ...
        tuples2 = new ArrayList<ArrayList<Integer>>();
        f2 = SystemTestUtil.createRandomHeapFile(10, 10000, 20, null, tuples2, "c");
    ...
    private double[] getRandomJoinCosts(EfficientJoinOptimizer jo, LogicalJoinNode js, int[] card1s, int[] card2s,
    ...
    public void estimateJoinCostTest() throws ParsingException {
        ...
        EfficientJoinOptimizer jo;
        Parser p = new Parser();
        jo = new EfficientJoinOptimizer(p.generateLogicalPlan(tid, "SELECT * FROM " + tableName1 + " t1, " + tableName2
        ...
        checkJoinEstimateCosts(jo, equalsJoinNode);
        // 1 join 1
        jo = new EfficientJoinOptimizer(p.generateLogicalPlan(tid, "SELECT * FROM " + tableName1 + " t1, " + tableName1
        ...
        checkJoinEstimateCosts(jo, equalsJoinNode);
        // 2 join 2
        jo = new EfficientJoinOptimizer(p.generateLogicalPlan(tid, "SELECT * FROM " + tableName2 + " t1, " + tableName2
    ...
    private void checkJoinEstimateCosts(EfficientJoinOptimizer jo, LogicalJoinNode equalsJoinNode) {
    ...
    public void estimateJoinCardinality() throws ParsingException {
        TransactionId tid = new TransactionId();
        Parser p = new Parser();
        EfficientJoinOptimizer j = new EfficientJoinOptimizer(p.generateLogicalPlan(tid, "SELECT * FROM " + tableName2
         ...
         * Integer.toString(4), Predicate.Op.EQUALS), stats1.estimateTableCardinality(0.8),
         * stats2.estimateTableCardinality(0.2), false, false);
         * 
         * // We don't specify in what way statistics should be used to improve these estimates. // So, just require
         * that they not be entirely unreasonable. Assert.assertTrue(cardinality > 800); Assert.assertTrue(cardinality
         * <= 2000);
         * 
         * cardinality = j.estimateJoinCardinality(new LogicalJoinNode(tableName2, tableName1, Integer.toString(3),
         * Integer.toString(4), Predicate.Op.EQUALS), stats2.estimateTableCardinality(0.2),
         * stats1.estimateTableCardinality(0.8), false, false);
         * 
         * Assert.assertTrue(cardinality > 800); Assert.assertTrue(cardinality <= 2000);
         */

        cardinality = j.estimateJoinCardinality(new LogicalJoinNode("t1", "t2", "c" + Integer.toString(3), "c"
                + Integer.toString(4), Predicate.Op.EQUALS), stats1.estimateTableCardinality(0.8),
    ...
    @Test
    // (timeout = 60000)
    public void superHugeOrderJoinsTest() throws IOException, DbException, TransactionAbortedException,
            ParsingException {
        final int IO_COST = 103;
        ...
        HeapFile smallHeapFileU = createDuplicateHeapFile(smallHeapFileTuples, 2, "c");
        HeapFile smallHeapFileV = createDuplicateHeapFile(smallHeapFileTuples, 2, "c");
        HeapFile smallHeapFileW = createDuplicateHeapFile(smallHeapFileTuples, 2, "c");
        HeapFile smallHeapFileX = createDuplicateHeapFile(smallHeapFileTuples, 2, "c");
        HeapFile smallHeapFileY = createDuplicateHeapFile(smallHeapFileTuples, 2, "c");
        HeapFile smallHeapFileZ = createDuplicateHeapFile(smallHeapFileTuples, 2, "c");
        ...
        Database.getCatalog().addTable(smallHeapFileU, "u");
        Database.getCatalog().addTable(smallHeapFileV, "v");
        Database.getCatalog().addTable(smallHeapFileW, "w");
        Database.getCatalog().addTable(smallHeapFileX, "x");
        Database.getCatalog().addTable(smallHeapFileY, "y");
        Database.getCatalog().addTable(smallHeapFileZ, "z");
        ...
        stats.put("m", new TableStats(smallHeapFileG.getId(), IO_COST));
        stats.put("n", new TableStats(smallHeapFileG.getId(), IO_COST));
        stats.put("o", new TableStats(smallHeapFileC.getId(), IO_COST));
        stats.put("p", new TableStats(smallHeapFileD.getId(), IO_COST));
        stats.put("q", new TableStats(smallHeapFileE.getId(), IO_COST));
        stats.put("r", new TableStats(smallHeapFileF.getId(), IO_COST));
        stats.put("s", new TableStats(smallHeapFileG.getId(), IO_COST));
        stats.put("t", new TableStats(smallHeapFileG.getId(), IO_COST));
        stats.put("u", new TableStats(smallHeapFileG.getId(), IO_COST));
        stats.put("v", new TableStats(smallHeapFileG.getId(), IO_COST));
        stats.put("w", new TableStats(smallHeapFileG.getId(), IO_COST));
        stats.put("x", new TableStats(smallHeapFileG.getId(), IO_COST));
        stats.put("y", new TableStats(smallHeapFileG.getId(), IO_COST));
        stats.put("z", new TableStats(smallHeapFileG.getId(), IO_COST));
        ...
        filterSelectivities.put("u", 1.0);
        filterSelectivities.put("v", 1.0);
        filterSelectivities.put("w", 1.0);
        filterSelectivities.put("x", 1.0);
        filterSelectivities.put("y", 1.0);
        filterSelectivities.put("z", 1.0);
        ...
        nodes.add(new LogicalJoinNode("t", "u", "c0", "c0", Predicate.Op.EQUALS));
        nodes.add(new LogicalJoinNode("u", "v", "c1", "c1", Predicate.Op.EQUALS));
        nodes.add(new LogicalJoinNode("v", "w", "c0", "c0", Predicate.Op.EQUALS));
        nodes.add(new LogicalJoinNode("w", "x", "c1", "c1", Predicate.Op.EQUALS));
        nodes.add(new LogicalJoinNode("x", "y", "c0", "c0", Predicate.Op.EQUALS));
        nodes.add(new LogicalJoinNode("y", "z", "c1", "c1", Predicate.Op.EQUALS));
        nodes.add(new LogicalJoinNode("z", "bigTable", "c0", "c0", Predicate.Op.EQUALS));
        ...
        j = new EfficientJoinOptimizer(
                p.generateLogicalPlan(
                        tid,
                        "SELECT COUNT(a.c0) FROM bigTable, a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x, y, z WHERE bigTable.c0 = z.c0 AND a.c1 = b.c1 AND b.c0 = c.c0 AND c.c1 = d.c1 AND d.c0 = e.c0 AND e.c1 = f.c1 AND f.c0 = g.c0 AND g.c1 = h.c1 AND h.c0 = i.c0 AND i.c1 = j.c1 AND j.c0 = k.c0 AND k.c1 = l.c1 AND l.c0 = m.c0 AND m.c1 = n.c1 AND n.c1 = o.c1 AND o.c0 = p.c0 AND p.c1 = q.c1 AND q.c0 = r.c0 AND r.c1 = s.c1 AND s.c0 = t.c0 AND t.c1 = u.c1 AND u.c0 = v.c0 AND v.c1 = w.c1 AND w.c0 = x.c0 AND x.c1 = y.c1 AND y.c0 = z.c0;"),
                nodes);
     ...
     */
    @Test
    public void nonequalityOrderJoinsTest() throws IOException, DbException, TransactionAbortedException,
            ParsingException {
        final int IO_COST = 103;
