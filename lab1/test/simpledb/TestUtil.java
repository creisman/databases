...
public class TestUtil {
     ...
     * @param tupdata
     *            an array such that the ith element the jth tuple lives in slot j * width + i
     ...
     *            the number of fields in each tuple
     * @param tupdata
     *            an array such that the ith element the jth tuple lives in slot j * width + i. Objects can be strings
     *            or ints; tuples must all be of same type.
     * @require tupdata.length % width == 0
     * @throws DbException
     *             if we encounter an error creating the TupleIterator
     */
    ...
    public static void matchAllTuples(DbIterator expected, DbIterator actual) throws DbException,
        ...
        // ADDED BY CREISMAN TO PREVENT SUPERSET
        expected.rewind();
        actual.rewind();

        while (actual.hasNext()) {
            Tuple expectedTup = actual.next();
            matched = false;
            expected.rewind();

            while (expected.hasNext()) {
                Tuple next = expected.next();
                if (compareTuples(expectedTup, next)) {
                    matched = true;
                    break;
                }
    ...
    static class LockGrabber extends Thread {
        ...
        /**
         * @param tid
         *            the transaction on whose behalf we want to acquire the lock
         * @param pid
